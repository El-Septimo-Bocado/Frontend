...
async function crearOrdenYRedirigir(){
  if(!(draft?.showtimeId) || !(draft?.holdId) || !(draft?.asientos?.length)) {
    ...
    // usar ruta robusta a reservacion.html en la misma carpeta
    window.location.href = new URL("./reservacion.html", window.location.href).href;
    return;
  }
  ...
  // Redirección a recibo.html en la misma carpeta
  const RECIBO_URL = new URL("./recibo.html", window.location.href).href;
  window.location.href = RECIBO_URL;
}
